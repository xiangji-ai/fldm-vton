...
class DDPM(pl.LightningModule):
    ...
    def get_input(self, batch, k):
        if k == "inpaint":
            ...
            inpaint = batch['im_warp_mask'] #im_warp_mask im_mask
            reference = batch['cloth_c']
            reference_detail=batch['cloth_warp_mask']
            cloth_position= batch['cloth_position']
            im_cloth_position=batch['im_cloth_position']
        ...
        reference_detail = reference_detail.to(memory_format=torch.contiguous_format).float()
        cloth_position = cloth_position.to(memory_format=torch.contiguous_format).float()
        im_cloth_position = im_cloth_position.to(memory_format=torch.contiguous_format).float()
        cloth_mask = cloth_mask.to(memory_format=torch.contiguous_format).float()

        return x,inpaint,mask,reference,reference_detail,cloth_position,im_cloth_position,cloth_mask
...
class LatentDiffusion(DDPM):
    ...
    def __init__(self,
        ...
        # takeoff_dir='/data/chwang/VITONLOG/pretrained/tk_upper.pth'
        # takeoff_dir='/data/chwang/VITONLOG/pretrained/tk_lower.pth'
        # takeoff_dir='/data/chwang/VITONLOG/pretrained/tk_dress.pth'
        # takeoff_dir = '/data/chwang/VITONLOG/pretrained/warp_epoch_100.pth'
        # print('initiaze the takeoff module from: {}'.format(takeoff_dir))
        # takeoff_statedict=torch.load(takeoff_dir,map_location='cpu')
        # self.takeoffmodule.load_state_dict(takeoff_statedict,strict=False)
        # for param in self.takeoffmodule.parameters():
        #     param.requires_grad = False
        # self.takeoffmodule.eval()
        # self.takeoffmodule.cuda()
    ...
    def instantiate_cond_stage(self, config):
        ...
        if not self.cond_stage_trainable:
            ...
            elif config == "__dino__":
                print("dino initialization...")
                # self.cond_stage_model = torch.hub.load('facebookresearch/dinov2', 'dinov2_vitl14')
                # torch.hub.set_dir('/home/chwang/.cache/torch/hub/facebookresearch_dinov2_main')
                self.cond_stage_model=torch.hub.load('/home/chwang/.cache/torch/hub/facebookresearch_dinov2_main', 'dinov2_vitl14',source='local')
    ...
    def get_learned_conditioning(self, c):
        if self.cond_stage_forward is None:
            if hasattr(self.cond_stage_model, 'encode') and callable(self.cond_stage_model.encode):
                ...
                c = self.cond_stage_model(c).unsqueeze(1)
    ...
    def get_input(self, batch, k, return_first_stage_outputs=False, force_c_encode=False,
                  cond_key=None, return_original_cond=False, bs=None,get_mask=False,get_reference=False,get_takeoff=False):

        x,inpaint,mask,reference,reference_detail,cloth_position,im_cloth_position,cloth_mask = super().get_input(batch, k)
        if bs is not None:
            x = x[:bs]
            inpaint = inpaint[:bs]
            mask = mask[:bs]
            reference = reference[:bs]
            reference_detail=reference_detail[:bs]
        #     original diffision
        x = x.to(self.device)
        encoder_posterior = self.encode_first_stage(x)
        z = self.get_first_stage_encoding(encoder_posterior).detach()
        ...
        #     cloth diffusion
        # encoder_posterior_reference = self.encode_first_stage(reference_detail)
        encoder_posterior_reference = self.encode_first_stage(reference_detail)
        z_cloth = self.get_first_stage_encoding(encoder_posterior_reference).detach()
        encoder_posterior_cloth_cond = self.encode_first_stage(cloth_mask)
        z_cloth_cond = self.get_first_stage_encoding(encoder_posterior_cloth_cond).detach()
        mask_resize = Resize([z.shape[-2],z.shape[-1]])(mask)
        z_new = torch.cat((z,z_inpaint,mask_resize),dim=1) #,z_cloth
        if self.model.conditioning_key is not None:
            ...
            if not self.cond_stage_trainable or force_c_encode:
                if isinstance(xc, dict) or isinstance(xc, list):
                    ...
                    c = self.proj_out(c)
                    # print("projct shape {}".format(c.shape))
                    c = c.float()
            else:
                c = xc
            if bs is not None:
        ...
        if get_takeoff:
            out.append({'cloth_position':cloth_position,
                        'im_cloth_position':im_cloth_position,
    ...
    def shared_step(self, batch, **kwargs):
        x, z_cloth,c,takeoff_output = self.get_input(batch, self.first_stage_key,get_takeoff=True)

        loss = self(x,z_cloth, c,takeoff_output)
    ...
    def get_takeoff_loss(self,takeoff_output, imcloth,person_clothes, t):
        _0, last_flow, _1, _2, _3, x_all, _4, _5 = takeoff_output
        # time step threshold for calculating losses
        last_flow = F.interpolate(last_flow, size=(512, 512), mode='bilinear')
    ...
    def forward(self, x, z_cloth, c, takeoff_input, *args, **kwargs):
        ...
        if self.current_epoch >=0:
            if len(takeoff_input['im_cloth_position'].shape)==3:
                im_cloth_position = takeoff_input['im_cloth_position'].unsqueeze(1)
            else:
    ...
    def reconp_losses(self, x_start, cond, t, zcloth=None):
        noise = torch.randn_like(x_start[:, :4, :, :])
        x_noisy = self.q_sample(x_start=zcloth, t=t, noise=noise)
        x_noisy = torch.cat((x_noisy, x_start[:, 4:, :, :]), dim=1)
        model_output = self.apply_model(x_noisy, t, cond)
    ...
    @torch.no_grad()
    def sample_log(self,x_T,cond,batch_size,ddim, ddim_steps,**kwargs):

        if ddim:
            ddim_sampler = DDIMSampler(self)
            shape = (self.channels, 512, 512)
    ...
    def log_images(self, batch, N=4, n_row=4, sample=True, ddim_steps=50, ddim_eta=1., return_keys=None,
        ...
        z,z_cloth, c, x, xrec, xc, mask, reference = self.get_input(batch, self.first_stage_key,
        ...
        if plot_diffusion_rows:
            ...
            for t in range(self.num_timesteps):
                if t % self.log_every_t == 0 or t == self.num_timesteps - 1:
                    ...
                    t = t.to(self.device).long()
                    # modified chwang
                    noise = torch.randn_like(z_start[:, :4, :, :])
                    # noise= z_cloth
                    z_noisy = self.q_sample(x_start=z_start[:, :4, :, :], t=t, noise=noise)
                    z_noisy = torch.cat((z_noisy, z_start[:, 4:, :, :]), dim=1)

                    # noise = z_cloth
                    z_noisy2 = self.q_sample(x_start=z_cloth, t=t, noise=noise)
            ...
            diffusion_row2 = torch.stack(diffusion_row2)  # n_log_step, n_row, C, H, W
            diffusion_grid2 = rearrange(diffusion_row2, 'n b c h w -> b n c h w')
            diffusion_grid2 = rearrange(diffusion_grid2, 'b n c h w -> (b n) c h w')
            diffusion_grid2 = make_grid(diffusion_grid2, nrow=diffusion_row2.shape[0])
            log["diffusion_row"] = diffusion_grid
            log["diffusion_row2"] = diffusion_grid2
        ...
        if sample:
            ...
            with self.ema_scope("Plotting"):
                if self.first_stage_key=='inpaint':
                    ...
                    z_cloth= self.q_sample(z_cloth, ts)
                    samples, z_denoise_row = self.sample_log(x_T=z_cloth,cond=c,batch_size=N,ddim=use_ddim,
            ...
            if inpaint:
                ...
                with self.ema_scope("Plotting Inpaint"):
                    ...
                    samples, _ = self.sample_log(cond=c,batch_size=N,ddim=use_ddim, eta=ddim_eta,
                                                ddim_steps=ddim_steps, x0=z[:N,:4], mask=mask,rest=z[:N,4:,:,:])
                x_samples = self.decode_first_stage(samples.to(self.device))
